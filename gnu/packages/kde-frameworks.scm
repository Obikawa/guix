         ...
         (replace 'check
           (lambda* (#:key tests? #:allow-other-keys)
             ...
             (when tests?
               ...
               ;;"Jim Knopf\nLummerlandstr. 1\n12345 Lummerstadt\n\nGERMANIA"
               (invoke "ctest" "-E"
			   "(kcontacts-addresstest|kcontacts-emailtest|kcontacts-testroundtrip|kcontacts-impptest|kcontacts-birthdaytest|kcontacts-addresseetest)")))))))
    (home-page "https://community.kde.org/Frameworks")
         ...
         (replace 'check
           (lambda* (#:key tests? #:allow-other-keys)
             (when tests? ;; Maybe locale issues with tests?
               (setenv "QT_QPA_PLATFORM" "offscreen")
               (invoke "ctest" "-E"
			   "(kateview_test|completion_test|movingrange_test)"))))
         (add-after 'install 'add-symlinks
