...
(define-public linux-libre-5.16-version "5.16.11")
...
(define-public linux-libre-5.16-pristine-source
  (let ((version linux-libre-5.16-version)
        (hash (base32 "08xhm3ngg9157r69v44akp6cj73g33l6wa7073s4sjn4lic6263d")))
...
(define-public linux-libre-5.10-version "5.10.102")
...
(define-public linux-libre-5.4-version "5.4.181")
...
(define-public linux-libre-5.4-pristine-source
  (let ((version linux-libre-5.4-version)
        (hash (base32 "0awsv38cclk7npp6q7kb5n0dsdic3k8xnqdsz780yl92dh6s0qzw")))
   (make-linux-libre-source version
                            (%upstream-linux-source version hash)
                            deblob-scripts-5.4)))

(define-public linux-libre-4.19-version "4.19.231")
...
(define-public linux-libre-4.19-pristine-source
  (let ((version linux-libre-4.19-version)
        (hash (base32 "0cc1ipar37gvzr1xdbda5hghnvjmy8pk8an8hs2jspmhnfrj6cb7")))
    (make-linux-libre-source version
                             (%upstream-linux-source version hash)
                             deblob-scripts-4.19)))

(define-public linux-libre-4.14-version "4.14.268")
...
(define-public linux-libre-4.9-version "4.9.303")
     ...
     `(#:configure-flags (list "--localstatedir=/var")
       ...
       (modify-phases %standard-phases
         ...
         (add-before 'install 'no-polkit-magic
           (lambda* (#:key inputs outputs #:allow-other-keys)
             (let ((dbus (string-append (assoc-ref outputs "out")
                                        "/share/dbus-1/system.d")))
               (mkdir-p dbus)
               (install-file (string-append (assoc-ref inputs "source")
                                            "/data/org.freedesktop.bolt.conf") dbus)
               ;; Meson ‘magically’ invokes pkexec, which fails (not setuid).
               (setenv "PKEXEC_UID" "something")))))))
    (native-inputs (list pkg-config `(,glib  "bin") asciidoc umockdev))
